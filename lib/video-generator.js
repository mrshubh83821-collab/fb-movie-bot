...
export async function generateSlideshowReel({ slides, audioPath, outputPath, tmpDir }) {
  ...
  // Use just the filename (not the full path) - ffmpeg resolves relative paths
  // in a concat list relative to the list file's own directory, so including
  // tmpDir again here would double it up (a bug that caused earlier failures).
  fs.writeFileSync(listFile, clipPaths.map((p) => `file '${path.basename(p)}'`).join("\n"));
